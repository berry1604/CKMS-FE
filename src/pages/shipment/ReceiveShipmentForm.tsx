...
export const ReceiveShipmentForm = ({ shipmentId, storeOrderIds, status, onCancel, onSubmit, isSubmitting }: ComponentProps) => {
    ...
    const [productDetails, setProductDetails] = useState<Record<number, ProductResponse>>({});
    const [receivedQuantities, setReceivedQuantities] = useState<Record<string, number>>({}); // key format: orderId-productId
            ...
            try {
                ...
                fetchedOrders.forEach(order => {
                    order.orderDetails?.forEach(detail => {
                        ...
                        initialReceived[`${order.orderId}-${detail.productId}`] = detail.quantity;
                    });
                });
    ...
    const handleQuantityChange = (orderId: number, productId: number, val: string) => {
    ...
    function ordresWithShortage() {
        ...
        orders.forEach(order => {
            const hasShort = order.orderDetails?.some(d => {
                const received = receivedQuantities[`${order.orderId}-${d.productId}`] ?? d.quantity;
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-1000">
                ...
                <div className="relative z-10 flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-zinc-400 font-bold leading-relaxed uppercase tracking-[0.1em]">
                            Vui lòng kiểm đếm thực tế số lượng hàng hóa nhận được theo từng đơn hàng dưới đây.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-12">
                {orders.map((order) => (
                    <div key={order.orderId} className="bg-zinc-900/10 border border-zinc-800/50 rounded-[40px] overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-700">
                        {/* Order Header */}
                        <div className="px-10 py-6 bg-gradient-to-r from-zinc-900/80 to-transparent border-b border-zinc-800/50 flex justify-between items-center">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em] mb-0.5">Mã đơn hàng</h4>
                                    <span className="text-xl font-black text-white tracking-tighter italic">#{order.orderId}</span>
                                </div>
                            </div>
                            <div className="text-right">
                                <div className="text-[9px] font-black text-zinc-600 uppercase tracking-widest mb-1">Ngày yêu cầu</div>
                            ...
                            <table className="w-full text-left text-sm whitespace-nowrap border-collapse">
                                <thead>
                                    <tr className="bg-black/20 border-b border-zinc-800/30">
                                        <th className="px-10 py-5 text-[9px] font-black text-zinc-500 uppercase tracking-[0.3em]">Sản phẩm & Hình ảnh</th>
                                        <th className="px-6 py-5 text-[9px] font-black text-zinc-500 uppercase tracking-[0.3em] text-center">Yêu cầu (Dự kiến)</th>
                                        <th className="px-6 py-5 text-[9px] font-black text-zinc-500 uppercase tracking-[0.3em] text-center w-48">Thực Nhận</th>
                                        <th className="px-6 py-5 text-[9px] font-black text-zinc-500 uppercase tracking-[0.3em] text-center">Tình trạng</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-zinc-800/40">
                                    {order.orderDetails?.map((item) => {
                                        const received = receivedQuantities[`${order.orderId}-${item.productId}`] ?? item.quantity;
                                        ...
                                        return (
                                            <tr 
                                                ...
                                                className={cn(
                                                    ...
                                                    isMissing ? 'bg-red-500/[0.04] hover:bg-red-500/[0.08]' : 'hover:bg-amber-500/[0.03]'
                                                )}
                                            >
                                                <td className="px-10 py-6">
                                                    <div className="flex items-center gap-5">
                                                        <div className="w-16 h-16 rounded-2xl bg-zinc-950 border border-zinc-800 overflow-hidden shadow-lg group-hover/row:scale-105 transition-transform duration-500">
                                                            {details?.imageUrl ? (
                                                                <img src={details.imageUrl} alt={item.productName} className="w-full h-full object-cover" />
                                                            ) : (
                                                        ...
                                                        <div>
                                                            <span className="block font-black text-white text-base tracking-tight mb-1">{item.productName}</span>
                                                            <div className="flex items-center gap-2">
                                                                <Badge variant="default" className="text-[9px] font-black tracking-widest h-5 border-zinc-800/50 bg-zinc-800/30">
                                                                    {details?.category?.name || 'Sản phẩm'}
                                                                </Badge>
                                                                <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-tighter">Đơn giá: {item.unitPrice.toLocaleString()}đ</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-6 py-6 text-center">
                                                    <div className="relative group/input max-w-[140px] mx-auto">
                                                        <input
                                                            ...
                                                            onChange={(e) => handleQuantityChange(order.orderId, item.productId, e.target.value)}
                                                            min={0}
                                                            disabled={isReadOnly}
                                                        />
                                                    </div>
            ...
            {hasShortage && (
                <div className="bg-red-950/20 border border-red-500/20 p-8 rounded-[40px] flex gap-6 items-start animate-in fade-in zoom-in-95 duration-700 shadow-[0_20px_60px_-20px_rgba(239,68,68,0.2)]">
                    ...
                    <div className="space-y-2">
                        <p className="text-sm font-black text-red-400 uppercase tracking-[0.2em]">Phát hiện thiếu hàng tại Đơn #{shortageInfo.join(', #')}</p>
                        <p className="text-[11px] text-zinc-500 font-bold leading-relaxed uppercase tracking-[0.1em]">
                            Số lượng thực nhận thấp hơn yêu cầu. Hệ thống đã ghi nhận sai lệch. Vui lòng ghi rõ lý do vào phần ghi chú bên dưới để hoàn tất biên bản đồng kiểm.
                        </p>
                    </div>
                </div>
            )}
