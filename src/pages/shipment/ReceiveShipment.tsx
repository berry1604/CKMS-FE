...
export const ReceiveShipment = () => {
    ...
    return (
        <div className="min-h-screen bg-[var(--bg-root)] pb-20">
            {/* Cinematic Header */}
            <div className="relative h-[320px] w-full overflow-hidden group/header">
                ...
                <div className="absolute inset-0 flex flex-col justify-end px-8 pb-12 max-w-7xl mx-auto w-full">
                    <div className="flex items-center gap-4 mb-3">
                        ...
                        <span className="text-amber-500 font-black tracking-[0.3em] text-[10px] uppercase italic">Điều phối nhập kho</span>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-end gap-6">
                        ...
                        <div className="px-6 py-3 backdrop-blur-3xl bg-white/5 border border-white/10 rounded-2xl shadow-2xl">
                            <span className="text-[var(--text-secondary)] text-[10px] uppercase tracking-[0.2em] block mb-1 font-black">Tổng chuyến hàng</span>
                            <span className="text-2xl font-black text-[var(--text-primary)] leading-none">{filteredShipments.length}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-8 -mt-10 relative z-10 space-y-8">
                {/* Search & Filter Bar */}
                <div className="backdrop-blur-3xl bg-[var(--bg-card)]/40 border border-[var(--border-primary)] rounded-[32px] p-4 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-center justify-between relative z-10">
                        <div className="relative w-full md:w-[400px] group/search">
                            <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none transition-transform group-focus-within/search:scale-110">
                                <Search size={18} className="text-[var(--text-secondary)] group-focus-within/search:text-amber-500 transition-colors" />
                            </div>
                            <input
                                type="text"
                                placeholder="Tìm mã chuyến, tên tài xế..."
                                className="w-full pl-12 pr-4 py-4 bg-[var(--bg-root)]/40 border border-[var(--border-primary)] rounded-2xl text-[var(--text-primary)] focus:outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500/40 transition-all duration-300 placeholder:text-[var(--text-secondary)]/50 hover:bg-[var(--bg-root)]/60"
                                value={searchQuery}
                        ...
                        <div className="flex items-center gap-3 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 no-scrollbar">
                            <div className="flex items-center gap-2 pr-4 border-r border-[var(--border-primary)] mr-2">
                                <Filter size={16} className="text-amber-500" />
                                <span className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest whitespace-nowrap">HIỂN THỊ</span>
                            </div>
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        statusFilter === status
                                            ...
                                            : "bg-[var(--bg-root)]/50 border-[var(--border-primary)] text-[var(--text-secondary)] hover:border-[var(--text-secondary)]/30 hover:text-[var(--text-primary)]"
                                    )}
                                >
                ...
                <div>
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center py-32 gap-6 scale-110">
                            ...
                            <span className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-[0.4em] animate-pulse">Luân chuyển dữ liệu...</span>
                        </div>
                    ) : filteredShipments.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-32 text-center animate-in fade-in zoom-in-95 duration-500">
                            <div className="w-24 h-24 rounded-[32px] bg-[var(--bg-card)]/50 border border-[var(--border-primary)] flex items-center justify-center mb-8 shadow-inner">
                                <Package size={40} className="text-[var(--text-secondary)]/30" />
                            </div>
                            <h3 className="text-xl font-bold text-[var(--text-secondary)] uppercase tracking-tighter mb-3">Hệ thống trống</h3>
                            <p className="text-[var(--text-secondary)]/60 max-w-sm mx-auto font-light italic">Không tìm thấy chuyến hàng nào khớp với điều kiện hiện tại.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {filteredShipments.map((shipment, idx) => (
                                <div
                                    ...
                                    className="group relative backdrop-blur-3xl bg-[var(--bg-card)]/40 border border-[var(--border-primary)] rounded-[40px] p-6 shadow-2xl transition-all duration-500 hover:border-amber-500/40 hover:-translate-y-2 hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.3)] animate-in fade-in slide-in-from-bottom-8 flex flex-col"
                                ...
                                >
                                    <div className="relative z-10 flex-1">
                                        <div className="flex justify-between items-start mb-8">
                                            <div className="flex items-center gap-4">
                                                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 p-px">
                                                    <div className="w-full h-full bg-[var(--bg-card)] rounded-2xl flex items-center justify-center text-amber-500 group-hover:bg-transparent group-hover:text-black transition-all">
                                                        <Truck size={24} />
                                                    </div>
                                                </div>
                                                <div>
                                                    <h3 className="text-2xl font-black text-[var(--text-primary)] tracking-tighter">#{shipment.shipmentId}</h3>
                                        ...
                                        <div className="space-y-4 mb-8">
                                            <div className="p-5 rounded-3xl bg-[var(--bg-root)]/50 border border-[var(--border-primary)] group-hover:border-[var(--text-secondary)]/20 transition-colors relative overflow-hidden">
                                                <div className="absolute top-0 left-0 w-1 h-full bg-amber-500 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-full bg-[var(--bg-card)] flex items-center justify-center text-[var(--text-secondary)]">
                                                        <User size={18} />
                                                    </div>
                                                    <div>
                                                        <div className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-[0.2em] mb-1">Cơ trưởng chuyến hàng</div>
                                                        <div className="text-sm font-bold text-[var(--text-primary)] tracking-tight">{shipment.driverName || 'Vô danh'}</div>
                                                        <div className="text-[10px] text-amber-500/80 font-mono mt-0.5">{shipment.driverPhone || '---'}</div>
                                                    </div>
                                                </div>
                                            </div>

                                            <div className="flex items-center gap-3 px-5 py-3 rounded-2xl bg-[var(--bg-root)]/30 border border-[var(--border-primary)] text-[var(--text-secondary)]">
                                    ...
                                    <Button
                                        ...
                                        className={cn(
                                            ...
                                            shipment.status === 'IN_TRANSIT'
                                                ...
                                                : "bg-[var(--bg-root)] text-[var(--text-secondary)] border border-[var(--border-primary)] hover:border-[var(--text-secondary)]/30 hover:text-[var(--text-primary)]"
                                        )}
                                    >
