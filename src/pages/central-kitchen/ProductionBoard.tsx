...
import {
    PlayCircle, CheckCircle, CheckSquare, Clock,
    HelpCircle, LayoutDashboard, Database,
    ArrowRight, Info, AlertCircle, CheckCircle2,
    Calendar, ClipboardList, Timer, Package
} from 'lucide-react';
...
export const ProductionBoard = () => {
    ...
    const [finishingPlanVersion, setFinishingPlanVersion] = useState<number | undefined>();
    const [planDetailForYield, setPlanDetailForYield] = useState<ProductionPlanDetailResponse | null>(null);
    ...
    const readyPlans = plans.filter(p => p.status === 'READY_TO_PRODUCE' || p.status === 'APPROVED');
    const inProdPlans = plans.filter(p => p.status === 'IN_PRODUCTION' || p.status === 'PLANNING');
    const donePlans = plans.filter(p => p.status === 'COMPLETED' || p.status === 'FINISHED' || p.status === 'PRODUCED');

    const renderCard = (plan: ProductionPlanSummaryResponse, columnType: 'READY' | 'IN_PROD' | 'DONE') => {
        const statusColors = {
            'READY': 'border-indigo-500/30 bg-indigo-500/5 hover:border-indigo-500',
            'IN_PROD': 'border-amber-500/30 bg-amber-500/5 hover:border-amber-500 ring-1 ring-amber-500/20 shadow-lg shadow-amber-900/10',
            ...
            <div key={plan.planId} className={cn(
                ...
                <div className="flex justify-between items-start mb-4">
                    <div className="flex flex-col">
                        ...
                        <h4 className="font-black text-zinc-100 mt-2 leading-none uppercase tracking-tighter text-sm group-hover:text-amber-500 transition-colors">{plan.planName}</h4>
                    </div>
                    {columnType === 'IN_PROD' && (
                ...
                <div className="space-y-3 pb-4 border-b border-zinc-800/50">
                    <div className="flex items-center justify-between text-[10px] font-bold text-zinc-600 uppercase tracking-tight">
                        ...
                        <span className="text-zinc-400 font-mono tracking-tighter">{plan.batchCode || 'N/A'}</span>
                    </div>
                    <div className="flex items-center justify-between text-[10px] font-bold text-zinc-600 uppercase tracking-tight">
                        <span>Ngày tạo:</span>
                        <span className="text-zinc-400">{new Date(plan.createdAt).toLocaleDateString('vi-VN')}</span>
                    </div>
                </div>
                ...
                <div className="pt-4 h-12 flex items-center">
                    {columnType === 'READY' && (
                        ...
                        >
                            <PlayCircle size={14} className="mr-2" strokeWidth={3} /> Bắt đầu nấu
                        </Button>
                    )}
                    {columnType === 'IN_PROD' && (
                        <Button
                        ...
                        >
                            <CheckCircle2 size={14} className="mr-2" strokeWidth={3} /> Hoàn tất mẻ
                        </Button>
                    )}
                    {columnType === 'DONE' && (
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 shrink-0">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <Badge variant="orange" className="text-[10px] font-black tracking-widest px-2 py-0 border-0 h-4 uppercase">PRODUCTION</Badge>
                        <h1 className="text-3xl font-black text-zinc-100 uppercase tracking-tighter">Bảng Điều Khiển Bếp</h1>
                    </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        <LayoutDashboard size={20} className={isLoading ? "animate-spin" : ""} />
                    </Button>
                </div>
            </div>
                ...
                <div className="flex flex-col bg-zinc-900/40 rounded-[32px] border border-zinc-800/50 overflow-hidden">
                    <div className="p-6 bg-zinc-900/80 backdrop-blur-md border-b border-zinc-800 flex justify-between items-center shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="font-black text-zinc-200 uppercase tracking-widest text-[11px]">Sẵn sàng sản xuất</h3>
                        </div>
                        <Badge variant="info" className="text-[10px] font-black border-0">{readyPlans.length}</Badge>
                    </div>
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                        {readyPlans.length > 0 ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center gap-4 opacity-10">
                                <Database size={48} />
                                <span className="text-[10px] font-black uppercase tracking-[0.2em]">Trống dữ liệu</span>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="flex flex-col bg-amber-500/[0.02] rounded-[32px] border border-amber-500/20 overflow-hidden ring-1 ring-amber-500/10 shadow-[0_0_50px_rgba(245,158,11,0.05)]">
                    <div className="p-6 bg-amber-500/5 backdrop-blur-md border-b border-amber-500/20 flex justify-between items-center shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="font-black text-amber-500 uppercase tracking-widest text-[11px]">Đang nấu</h3>
                        </div>
                        <Badge variant="orange" className="text-[10px] font-black border-0">{inProdPlans.length}</Badge>
                    </div>
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                        {inProdPlans.length > 0 ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center gap-4 opacity-10">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-500">Chưa bắt đầu mẻ nào</span>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="flex flex-col bg-zinc-900/40 rounded-[32px] border border-zinc-800/50 overflow-hidden opacity-60">
                    <div className="p-6 bg-zinc-900/80 backdrop-blur-md border-b border-zinc-800 flex justify-between items-center shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="font-black text-zinc-200 uppercase tracking-widest text-[11px]">Hoàn thành hôm nay</h3>
                        </div>
                        <Badge variant="success" className="text-[10px] font-black border-0">{donePlans.length}</Badge>
                    </div>
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                        {donePlans.length > 0 ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center gap-4 opacity-10">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">Trống</span>
                            </div>
                        )}
                    </div>
                </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        <Badge variant="orange" className="text-[8px] font-black tracking-[0.2em] h-4 py-0 border-0">YIELD REPORT</Badge>
                                        <span className="text-[10px] font-mono font-black text-zinc-600">ID: #{finishingPlanId}</span>
                                    </div>
                                    <h3 className="text-xl font-black text-zinc-100 uppercase tracking-tight mt-1">Nghiệm thu mẻ nấu</h3>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-8 space-y-6">
                            {isDetailLoading ? (
                                <div className="flex flex-col items-center justify-center py-12 gap-4">
                                    ...
                                    <span className="text-[11px] font-black text-zinc-600 uppercase tracking-widest">Đang đối soát dữ liệu...</span>
                                </div>
                            ) : planDetailForYield ? (
                                <div className="space-y-6 animate-in fade-in duration-500">
                                    <div className="bg-zinc-900/40 p-6 rounded-[24px] border border-zinc-800/50 space-y-3">
                                        <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">Tên mẻ:</span>
                                            <span className="text-sm font-black text-amber-500">{planDetailForYield.planName}</span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">Mã lô vạ:</span>
                                            <span className="text-sm font-black text-zinc-300 font-mono italic">{planDetailForYield.batchCode}</span>
                                        </div>
                                    </div>

                                    <div className="space-y-3">
                                        <div className="flex justify-between items-center ml-1">
                                            <h4 className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">Sản lượng dự kiến</h4>
                                            <Badge variant="info" className="text-[8px]">API Mocked</Badge>
                                        </div>
                                        <div className="border border-zinc-800 rounded-[24px] bg-black/40 p-4 divide-y divide-zinc-800/50">
                                            <div className="flex items-center justify-between py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-[13px] font-bold text-zinc-400">Tất cả sản phẩm đầu ra</span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <input
                                                        type="number"
                                                        ...
                                                        readOnly
                                                    />
                                                    <span className="text-[11px] font-black text-zinc-600 uppercase">Unit</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-5 rounded-[20px] bg-amber-500/5 border border-amber-500/10 flex gap-4">
                                        ...
                                        <p className="text-[11px] text-zinc-500 font-medium italic leading-relaxed">
                                            Lưu ý: Sau khi xác nhận hoàn thành, mẻ sản xuất sẽ chuyển trạng thái "PRODUCED" và sẵn sàng để quản trị kho điều hành phân phối (Allocation).
                                        </p>
                                    </div>
                                </div>
                            ) : (
