...
export const CreateProductionPlan = () => {
    ...
    const fetchKitchens = useCallback(async () => {
        ...
        try {
            ...
            if (user?.kitchenId) {
                const userKitchen = kitchenList.find(k => k.kitchenId === user.kitchenId);
                if (userKitchen) setSelectedKitchenId(userKitchen.kitchenId);
                else if (kitchenList.length > 0) setSelectedKitchenId(kitchenList[0].kitchenId);
    ...
    const fetchOrders = useCallback(async () => {
        ...
        try {
            const ordersRes = await storeOrderApi.getAllOrders({ status: 'APPROVED', size: 100 });
                        ...
                        <div className="lg:col-span-2 space-y-6 bg-zinc-900/40 p-8 rounded-[32px] border border-zinc-800/50">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3 font-inter">
                                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] ml-1">Chọn Bếp Thực Hiện</label>
                                    <div className="relative group">
                                        <ChefHat className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-amber-500 transition-colors" size={20} />
                                        <select
                    ...
                    <div className="flex flex-col lg:flex-row gap-8 animate-in slide-in-from-right-8 duration-500">
                                ...
                                <div className="overflow-x-auto max-h-[600px] custom-scrollbar">
                                    <table className="w-full text-left border-collapse">
                                        ...
                                        <tbody className="divide-y divide-zinc-800/30">
                                            {isLoadingOrders ? (
                                                <tr><td colSpan={6} className="px-8 py-20 text-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-amber-500 mx-auto"></div></td></tr>
                                            ) : orders.length === 0 ? (
                                                <tr><td colSpan={6} className="px-8 py-20 text-center text-zinc-600 font-bold uppercase tracking-widest opacity-30">Không có đơn hàng nào chờ gom mẻ</td></tr>
                                            ) : (
                                                orders.map(order => (
                                                    <tr
                                                        key={order.orderId}
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-4 py-5">
                                                            <div className="flex flex-col">
                                                                ...
                                                                <span className="text-[9px] text-zinc-600 font-medium italic">Đặt lúc: {new Date(order.orderDate).toLocaleTimeString('vi-VN')}</span>
                                                            </div>
                                                        </td>
                                                        <td className="px-4 py-5">
                                                        ...
                                                        <td className="px-4 py-5">
                                                            <div className="flex flex-wrap gap-1 max-w-[200px]">
                                                                {order.orderDetails?.slice(0, 2).map((i, idx) => (
                                                                    <span key={idx} className="text-[9px] px-1.5 py-0.5 bg-zinc-950/50 text-zinc-500 border border-zinc-800 rounded-md font-medium">
                                                                        {i.productName}
                                                                    </span>
                                                                ))}
                                                                {order.orderDetails && order.orderDetails.length > 2 && (
                                                                    <span className="text-[9px] px-1.5 py-0.5 text-zinc-600 font-bold">+{order.orderDetails.length - 2} món</span>
                                                                )}
                                                            </div>
                                                        </td>
                                                        <td className="px-8 py-5 text-right">
                                                            <span className="text-[13px] font-black text-zinc-100">{(order.totalAmount || 0).toLocaleString()} <span className="text-[9px] text-zinc-500 ml-0.5">đ</span></span>
                                                        </td>
                                                    </tr>
                                                ))
                                            )}
                        ...
                        <div className="lg:w-[320px] space-y-6">
                            <div className="bg-zinc-900/40 p-8 rounded-[32px] border border-zinc-800/50 sticky top-8 space-y-8 shadow-2xl">
                                <div>
                                    <h3 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-4">Chi tiết lựa chọn</h3>
                                    <div className="space-y-4">
                                        <div className="p-4 bg-zinc-950 rounded-2xl border border-zinc-800/50 space-y-3">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] font-black text-zinc-600 uppercase tracking-tighter">Bếp Thực Hiện</span>
                                                    <span className="text-xs font-black text-zinc-200 uppercase truncate max-w-[160px]">
                                                        {kitchens.find(k => k.kitchenId === selectedKitchenId)?.name || 'Chưa chọn'}
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] font-black text-zinc-600 uppercase tracking-tighter">Ngày kế hoạch</span>
                                        ...
                                        <div className="p-4 bg-zinc-950 rounded-2xl border border-zinc-800/50">
                                            <div className="flex justify-between items-center mb-1">
                                                <span className="text-[10px] font-black text-zinc-600 uppercase tracking-tighter">Đã chọn</span>
                                                <span className="text-xl font-black text-amber-500">{selectedOrderIds.size}</span>
                                            </div>
                                            <div className="w-full bg-zinc-900 h-1 rounded-full overflow-hidden">
                                                <div 
                                                    className="h-full bg-amber-500 transition-all duration-500" 
                                                    style={{ width: `${(selectedOrderIds.size / (orders.length || 1)) * 100}%` }}
                                                ></div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <p className="text-[9px] text-zinc-500 font-medium text-center px-4 italic leading-relaxed">
                                        Tiếp theo bạn sẽ xem tổng hợp nhu cầu sản xuất từ các đơn hàng đã chọn phía trên.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                {step === 3 && (
                    <div className="max-w-4xl mx-auto space-y-4 animate-in slide-in-from-right-8 duration-500">
                        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl relative">
                            ...
                            <div className="p-5 border-b border-zinc-800/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-zinc-900/50">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 className="text-xl font-black text-white uppercase tracking-tight">Kế hoạch sản xuất</h2>
                                        <div className="flex items-center gap-2 mt-0.5">
                                            <Badge variant="orange" className="text-[7px] px-1 py-0 h-3.5 border-0 font-black uppercase tracking-widest">Dự thảo</Badge>
                                            <span className="text-[9px] text-zinc-600 font-bold uppercase tracking-widest leading-none">Gom mẻ tổng hợp</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 bg-zinc-950/60 p-4 rounded-xl border border-zinc-800/50 min-w-full md:min-w-[480px]">
                                    <div className="space-y-0.5">
                                        <span className="text-[7px] font-black text-zinc-600 uppercase tracking-widest leading-none">Bếp thực hiện</span>
                                        <p className="text-[10px] font-black text-zinc-100 uppercase truncate max-w-[100px]">
                                            {kitchens.find(k => k.kitchenId === selectedKitchenId)?.name}
                                        </p>
                                    </div>
                                    <div className="space-y-0.5 border-l border-zinc-800/50 pl-4">
                                        <span className="text-[7px] font-black text-zinc-600 uppercase tracking-widest leading-none">Ngày sản xuất</span>
                                        <p className="text-[10px] font-black text-zinc-100 italic">
                                            {new Date(plannedDate).toLocaleDateString('vi-VN')}
                                        </p>
                                    </div>
                                    <div className="space-y-0.5 border-l border-zinc-800/50 pl-4">
                                        <span className="text-[7px] font-black text-zinc-600 uppercase tracking-widest leading-none">Điều phối viên</span>
                                        <p className="text-[10px] font-black text-zinc-100 uppercase truncate max-w-[90px]">
                                            {user?.name || '---'}
                                        </p>
                                    </div>
                                    <div className="space-y-0.5 border-l border-zinc-800/50 pl-4">
                                        <span className="text-[7px] font-black text-zinc-600 uppercase tracking-widest leading-none">Quy mô</span>
                                        <p className="text-[10px] font-black text-emerald-500 uppercase">{selectedOrderIds.size} Đơn hàng</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-6 space-y-4">
                                    ...
                                    <table className="w-full text-left">
                                        ...
                                        <tbody className="divide-y divide-zinc-800/30">
                                            {aggregatedDemand.map((item, i) => (
                                                <tr key={i} className="hover:bg-amber-500/[0.01] transition-colors group">
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-1.5 h-1.5 rounded-full border border-amber-500/30 group-hover:bg-amber-500 transition-all"></div>
                                                            <div className="flex flex-col">
                                                                <span className="text-[13px] font-black text-zinc-100 tracking-tight group-hover:text-amber-500 transition-colors uppercase">{item.name}</span>
                                                                <span className="text-[9px] text-zinc-700 font-bold tracking-widest uppercase">MÃ: #{item.productId}</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4 text-right">
                                                        <div className="inline-flex items-center gap-2 tabular-nums">
                                                            ...
                                                            <span className="text-[9px] font-black text-zinc-600 uppercase tracking-widest">Món</span>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))}
                                ...
                                <div className="bg-emerald-500/5 rounded-2xl border border-emerald-500/10 p-5 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Xác nhận tổng lượng</span>
                                            <span className="text-[9px] text-zinc-500 font-medium italic leading-none">Dựa trên {selectedOrderIds.size} đơn hàng đã duyệt.</span>
                                        </div>
                                    </div>
                                    <div className="flex flex-col items-end">
                                        ...
                                        <span className="text-[9px] font-black text-emerald-500/50 uppercase -mt-0.5 tracking-widest">Thành phẩm</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-4">
                            <Button variant="ghost" onClick={() => setStep(2)} disabled={isSubmitting} className="text-zinc-600 hover:text-white font-black uppercase text-[10px] tracking-widest h-10 transition-colors">
