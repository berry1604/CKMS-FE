...
import { DollarSign, AlertCircle, Filter, Eye, FileText, Layers } from 'lucide-react';
...
export const BillingList = () => {
    const [statements, setStatements] = useState<BillingStatementSummaryResponse[]>([]);
    ...
    const handleGenerateManual = async () => {
        ...
        try {
            ...
            const msg = error.response?.data?.message || 'Failed to generate billing statement';
            toast.error(msg);
        } finally {
    ...
    const handleGenerateBatch = async () => {
        ...
        try {
            ...
            const msg = error.response?.data?.message || 'Failed to generate batch billing statements';
            toast.error(msg);
        } finally {
            setIsGenerating(false);
        }
    ...
    const paidCount = statements.filter(s => s.status?.toUpperCase() === 'PAID').length;
    const overdueCount = statements.filter(s => s.status?.toUpperCase() === 'OVERDUE').length;

    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <h1 className="text-2xl font-bold text-gray-200 tracking-tight">Billing & Statements</h1>

                <div className="flex items-center gap-2">
                    <Button variant="outline" onClick={() => setShowManualModal(true)}>
                        <FileText size={16} className="mr-2" /> Generate Manual
                    </Button>
                    <Button className="bg-amber-600 hover:bg-amber-700 text-white" onClick={() => setShowBatchModal(true)}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="bg-zinc-900/50 border-0 shadow-sm ring-1 ring-zinc-800">
                    <div className="flex items-start justify-between p-4">
                        <div>
                            <p className="text-sm font-medium text-gray-400">Total Statements</p>
                            <p className="text-2xl font-bold text-gray-200 mt-1">{totalElements}</p>
                        </div>
                ...
                <Card className="bg-zinc-900/50 border-0 shadow-sm ring-1 ring-zinc-800">
                    <div className="flex items-start justify-between p-4">
                        <div>
                            <p className="text-sm font-medium text-gray-400">Paid (this page)</p>
                            <p className="text-2xl font-bold text-green-500 mt-1">{paidCount}</p>
                        </div>
                ...
                <Card className="bg-zinc-900/50 border-0 shadow-sm ring-1 ring-zinc-800">
                    <div className="flex items-start justify-between p-4">
                        <div>
                            <p className="text-sm font-medium text-gray-400">Overdue (this page)</p>
            ...
            {showManualModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <Card className="w-full max-w-md p-6 bg-zinc-900 shadow-xl border border-zinc-700">
                        <h2 className="text-lg font-bold text-gray-200 mb-4">Generate Manual Statement</h2>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-300 mb-1">Store ID</label>
                                <Input type="number" min="1" placeholder="e.g. 1" value={manualForm.storeId} onChange={e => setManualForm({ ...manualForm, storeId: e.target.value })} />
                            </div>
                            <div>
                        ...
                        <div className="flex justify-end gap-3 mt-6">
                            ...
                            <Button className="bg-amber-600 hover:bg-amber-700 text-white" onClick={handleGenerateManual} isLoading={isGenerating}>Generate</Button>
                        </div>
                    </Card>
                </div>
            )}
            ...
            {showBatchModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <Card className="w-full max-w-md p-6 bg-zinc-900 shadow-xl border border-zinc-700">
                        <h2 className="text-lg font-bold text-gray-200 mb-4">Generate Batch Statements</h2>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-300 mb-1">Cycle Name</label>
                                <Input placeholder="e.g. Q1-2026" value={batchForm.cycleName} onChange={e => setBatchForm({ ...batchForm, cycleName: e.target.value })} />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-300 mb-1">Period Start</label>
                        ...
                        <div className="flex justify-end gap-3 mt-6">
                            ...
                            <Button className="bg-amber-600 hover:bg-amber-700 text-white" onClick={handleGenerateBatch} isLoading={isGenerating}>Generate Batch</Button>
                        </div>
                    </Card>
                </div>
            )}
