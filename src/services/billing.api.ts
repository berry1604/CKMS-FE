...
export const billingApi = {
     ...
     */
    generateManualStatement: async (storeId: number, periodStart: string, periodEnd: string): Promise<BillingStatementResponse> => {
     ...
     * Security: hasAuthority('CONFIRM_PAYMENT')
     */
    generateBatchStatements: async (request: BatchBillingStatementRequest): Promise<BatchBillingStatementResponse> => {
    ...
    getStatements: async (params?: {
        ...
        const response = await axiosClient.get<ApiResponse<PageResponse<BillingStatementSummaryResponse>>>(
            '/billing-statements',
            { params }
        );
        return response.data.data;
    ...
    getStatementDetail: async (id: number): Promise<BillingStatementDetailResponse> => {
        const response = await axiosClient.get<ApiResponse<BillingStatementDetailResponse>>(
     ...
     * Security: hasAuthority('CONFIRM_PAYMENT')
     */
    payStatement: async (id: number, request: PaymentStatementRequest): Promise<PaymentStatementResponse> => {
        const response = await axiosClient.patch<ApiResponse<PaymentStatementResponse>>(
            `/billing-statements/${id}/pay`,
            request
        );
        return response.data.data;
    }
